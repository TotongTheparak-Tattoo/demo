...
function makeBase(columns) {
    ...
    return {
        ...
        productionTarget: {
        columns: ["Target/day", "Average plan_target/day", "Total target", "Total plan target"],
        rows: [
...
// ✅ เติม wos q'ty ใต้ "Plan target" จาก /data_menagement/plan_wos_qty/get
async function loadPlanWosQtyAndPatch() {
  try {
    ...
    const monthly = await res.json(); // [{month: <raw>, wosQty: number}, ...]
    ...
    setData(prev => {
      ...
      for (const row of monthly || []) {
        const key = toMonthKeyFromTargetPlanMonth(row?.targetPlanMonth) ?? toMonthKey(row?.targetPlanMonth);
        if (!key) continue;
      ...
      const rows = prev.rows.map((r, i) =>
        i === 3 && r.label === "wos q'ty" ? { ...r, values } : r
      );
      return { ...prev, rows };
    });
  } catch (e) {
...
// ใช้ผลรวม planTarget รายเดือนจาก /data_menagement/plan_target/get
async function loadPlanTargetAndPatch() {
  try {
    ...
    const monthly = await res.json(); // [{month:'YYYY/MM', totalPlanTarget: number}, ...]
...
useEffect(() => {
  ...
  async function loadBalanceOrder() {
    ...
    new Set((items || []).map(r => toMonthKeyFromTargetPlanMonth(r?.targetPlanMonth)).filter(Boolean))
    ...
    const colIndex = Object.fromEntries(columns.map((c, i) => [c, i]));
      ...
      for (const r of items || []) {
        ...
        let bucket =
          firstMonth && cur < firstMonth ? "PASS" :
          lastMonth && cur > lastMonth ? "OVER" :
          key in colIndex ? key : "OVER";
        ...
        // --- เก็บ WOS แบบไม่ซ้ำ
        // ถ้าฟิลด์ WOS ของคุณไม่ใช่ orderNo ให้เปลี่ยนบรรทัดนี้ เช่น r?.wosNo
  ...
  return () => { mounted = false; };
    ...
    const wosIdxs = [];
    copy.forEach((r, i) => { if (r.label === "wos q'ty") wosIdxs.push(i); });
    ...
    // ---- คำนวณ wos q'ty diff = (wosBalance - wosPlan) ----
                  ...
                  <TableCell
                    ...
                    sx={{
                      ...
                      ...(data.setup.columns[i] === "diff"
                        ? { fontWeight: 700, color: v < 0 ? "error.main" : v > 0 ? "success.main" : "text.primary" }
                        : {}),
                    }}
                  >
                    {fmt(v)}
